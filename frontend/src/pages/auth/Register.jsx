...
export default function Register() {
  ...
  const handleSubmit = async () => {
    ...
    // Map frontend form field names to backend API expected payload
    const payload = {
      role,
    ...
    if (role === "donor") {
      ...
      payload.organizationName = formData.orgName || "";
    } else {
      payload.name = formData.contactPerson;
      payload.organizationName = formData.ngoName;
      payload.registrationNumber = formData.regNumber;
      payload.verificationDocument = formData.verificationDoc || "";
    }
    ...
    <div className="min-h-screen w-full flex justify-center px-4 py-8" style={{ background: PAGE_BG }}>
      ...
      >
        ...
        <div className="flex justify-between items-center mb-2">
          ...
          <Link to="/auth/login" className="text-sm font-semibold no-underline" style={{ color: GREEN }}>
            Login
          </Link>
        </div>
        ...
        <div className="text-center text-[13px] mt-4" style={{ color: TEXT_MUTED }}>
          ...
          <Link to="/auth/login" className="font-semibold no-underline" style={{ color: GREEN }}>
            Login
          </Link>
        </div>
      </div>
